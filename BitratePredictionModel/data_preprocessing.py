...
def PreprocessData():
    ...
    query_result = client.query('SELECT * FROM ime_tablice')
    
    dataset = pd.read_json("") #treba path do json filea --> primjer dataset = pd.read_json("C:\\Users\\Marko\\Desktop\\tr2.json",None,"frame")
    ...
    #np.uint64 beacuse of adding polynomial features    
    transmit_bitrate_dataset = dataset['mean_txOutOctets'].to_numpy(dtype=np.uint64)
    receive_bitrate_dataset = dataset['mean_txInOctets'].to_numpy(dtype=np.uint64)
